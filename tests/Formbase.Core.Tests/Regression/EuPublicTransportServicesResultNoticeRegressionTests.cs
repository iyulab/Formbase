...
public sealed class EuPublicTransportServicesResultNoticeRegressionTests
{
    ...
    public async Task All_fixture_documents_are_accepted_and_projected()
    {
        ...
        {
            await engine.AcceptAsync(PublicTransportResultNoticeType, DocumentBody.Parse(notice.GetRawText()));
        }

        var result = await engine.ProjectAsync(PublicTransportResultNoticeType);
