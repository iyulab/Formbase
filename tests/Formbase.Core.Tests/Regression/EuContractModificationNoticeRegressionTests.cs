...
public sealed class EuContractModificationNoticeRegressionTests
{
    ...
    public async Task All_fixture_documents_are_accepted_and_projected()
    {
        ...
        {
            await engine.AcceptAsync(ModificationNoticeType, DocumentBody.Parse(notice.GetRawText()), cancellationToken: TestContext.Current.CancellationToken);
        }

        var result = await engine.ProjectAsync(ModificationNoticeType, TestContext.Current.CancellationToken);
