...
public sealed class MorphDbFixture : IAsyncLifetime
{
    ...
    private async Task StartServiceAsync(INetwork network)
    {
        ...
        // Pinned to the compatibility pair (Formbase.* 0.9.x ↔ MorphDB 0.11.x): CI must test the
        ...
        _morphdb = new ContainerBuilder(
                Environment.GetEnvironmentVariable("FORMBASE_MORPHDB_IMAGE") ?? "ghcr.io/iyulab/morphdb:0.11.0")
            .WithNetwork(network)
