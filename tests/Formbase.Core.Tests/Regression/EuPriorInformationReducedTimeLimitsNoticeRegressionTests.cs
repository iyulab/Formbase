...
public sealed class EuPriorInformationReducedTimeLimitsNoticeRegressionTests
{
    ...
    public async Task All_fixture_documents_are_accepted_and_projected()
    {
        ...
        {
            await engine.AcceptAsync(ReducedTimeLimitsNoticeType, DocumentBody.Parse(notice.GetRawText()));
        }

        var result = await engine.ProjectAsync(ReducedTimeLimitsNoticeType);
